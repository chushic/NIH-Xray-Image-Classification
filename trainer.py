...
def fit(device, XRayTrain_dataset, train_loader, val_loader, test_loader, model, model_name, 
                                         loss_fn, optimizer, losses_dict,
                                         epochs_till_now, epochs, 
                                         log_interval, save_interval, 
                                         lr, bs, test_only = False):
    '''
    ...
    for epoch in range(epochs):
        ...
        save_name = '{}_{}_{}'.format(model_name, str.split(str(lr), '.')[-1], str(epochs_till_now).zfill(2))

        # the follwoing piece of code needs to be worked on !!! LATEST DEVELOPMENT TILL HERE
